...
def render_video(mpidir, renderpathfile, outputfile, use_N):
    ...
    print('Loaded {} mpis, each with shape {}'.format(len(mpis),
                                                      mpis[0].mpi.shape))
    ...
    render_poses[:, :2,
                 -1] = np.round(render_poses[:, :2, -1] * args.crop_factor)
    print('Rendering {} poses, crop factor {}, res {}, mpi shape {}'.format(
        render_poses.shape[0], args.crop_factor,
        render_poses[0, :2, -1].astype(np.int32), mpis[0].mpi.shape))

    outframes = render_mpis(mpis,
                            render_poses,
                            use_N=use_N,
                            weight_scale=1.,
                            alpha_blend=True)
