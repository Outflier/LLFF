...
def tfmm(A, B):
    with tf.variable_scope('tfmm'):
        return tf.reduce_sum(A[..., :, :, tf.newaxis] *
                             B[..., tf.newaxis, :, :],
                             axis=-2)
...
def myposes2mats(poses, fix_yx=False):
    with tf.variable_scope('myposes2mats'):
        ...
        with tf.variable_scope('Kstuff'):
            ...
            K_inv = stk([
                1. / f, m_z, -w * .5 / f, m_z, 1. / f, -h * .5 / f, m_z, m_z,
                -m_o
            ], -1)
...
def plane_homogs(pose_t,
    ...
    with tf.variable_scope('plane_homogs'):
        ...
        H = tfmm(K_s[..., tf.newaxis, :, :],
                 tfmm(H, K_t_inv[..., tf.newaxis, :, :]))
        if y_flip:
            premat = tf.stack([1, 0, 0., 0, -1, pose_t[0, -1] - 1, 0, 0, 1.],
                              -1)
            premat = tf.cast(premat, dtype=T_t2s.dtype)
            premat = tf.reshape(premat, [3, 3])

            postmat = tf.stack([1, 0, 0., 0, -1, pose_s[0, -1] - 1, 0, 0, 1.],
                               -1)
...
def homog_warp(img, H, retcos=False, window=None):
    with tf.variable_scope('homog_warp'):
        ...
        coords = tf.meshgrid(tf.range(bds[0], bds[2]),
                             tf.range(bds[1], bds[3]),
                             indexing='ij')
        # coords = tf.meshgrid(tf.range(h), tf.range(w), indexing='ij')
        coords = tf.cast(tf.stack([coords[1], coords[0]], 0),
                         H.dtype)  # [2, H, W]

        coords_t = tf.concat([coords, tf.ones_like(coords[:1, ...])],
                             0)  # [3, H, W]
        coords_t = tf.reshape(H,
                              [-1, 3, 3, 1, 1]) * coords_t  # [-1, 3, 3, H, W]
...
def render_mpi_homogs(mpi_rgba,
    ...
    with tf.variable_scope('render_mpi_homogs'):
        ...
        weights = mpiR_alpha * tf.cumprod(
            1. - mpiR_alpha + ALPHA_EPS, -2, exclusive=True,
            reverse=True)  # goddamnit pratul
...
def make_psv_homogs(img, pose, newpose, dispvals, num_depths, window=None):
    with tf.variable_scope('make_cv_homogs'):
        ...
        img = tf.reshape(
            img,
            [-1, 1, img_sh[-3], img_sh[-2], img_sh[-1]])  # [N, 1, H, W, 3]
        img_tiled = tf.tile(
            img, [1, num_depths, 1, 1, 1])  # go in as [N, D, H, W, 3]
        img_tiled = tf.reshape(
            img_tiled,
            [-1, img_sh[-3], img_sh[-2], img_sh[-1]])  # [N*D, H, W, 3]

        cvd = homog_render_fn(img_tiled, H,
                              window=window)  # come out as [N*D, H, W, 3]
        h, w = img_sh[-3], img_sh[-2]
        if window is not None:
            h = tf.cond(window[3] > 0, lambda: window[2] - window[0],
                        lambda: h)
            w = tf.cond(window[3] > 0, lambda: window[3] - window[1],
                        lambda: w)
        cvd = tf.reshape(cvd,
                         [-1, num_depths, h, w, img_sh[-1]])  # [N, D, H, W, 3]
