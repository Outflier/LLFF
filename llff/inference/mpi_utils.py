...
def savempi(mpi, pose, dvals, basedir, txt_only=False, binary=False):
    ...
    if binary:
        x = (255 * np.clip(np.transpose(mpi, [2, 0, 1, 3]), 0, 1)).astype(
            np.uint8)
        ...
        for i in range(mpi.shape[-2]):
            ...
            imageio.imwrite(os.path.join(basedir, 'mpi{:02d}.png'.format(i)),
                            x)
...
class MPI:
    ...
    def generate(self, generator, num_planes):
        inputs = [[
            self.imgs,
            np.ones_like(self.imgs[..., 0]), self.poses, num_planes,
            self.cdepth, self.idepth
        ]]
    ...
    def save(self, basedir, txt_only=False, binary=False):
        ...
        with open(os.path.join(basedir, 'metadata.txt'), 'w') as file:
            file.write('{} {} {} {}\n'.format(mpi.shape[0], mpi.shape[1],
                                              mpi.shape[2], pose[2, -1]))
            for j in range(4):
                file.write('{} {} {}\n'.format(*[pose[i, j]
                                                 for i in range(3)]))
        ...
        if binary:
            ...
            for i in range(mpi.shape[-2]):
                x = (255 * np.clip(mpi[..., i, :], 0, 1)).astype(np.uint8)
                imageio.imwrite(
                    os.path.join(basedir, 'mpi{:02d}.png'.format(i)), x)
...
def run_inference(imgs,
    ...
    for i in range(imgs.shape[-1]):
        ...
        print(i, '(of {})'.format(imgs.shape[-1]), '<-', inds, 'depths',
...
def render_mpis(mpis,
    ...
    for i, p in enumerate(render_poses):
        ...
        if alpha_blend:
            outframe = (rends[..., :3] *
                        weights).sum(0) / (1e-10 +
                                           (rends[..., -1:] * weights).sum(0))
        else:
            outframe = ((rends[..., :3] / (1e-10 + rends[..., -1:])) *
                        weights).sum(0)
