...
class DeepIBR():
    ...
    def setup_renderer(self):
        ...
        with self.graph.as_default():
            ...
            mpi_rgba = tf.placeholder(tf.float32, [None, None, None, 4],
                                      name='mpi_rgba')
            ...
            self.render_args = [
                tf.expand_dims(mpi_rgba, 0), pose, newpose, 1. / inf_depth,
                1. / close_depth,
                tf.shape(mpi_rgba)[-2]
            ]
    ...
    def render_mpi(self, *args):
        ...
        rgb, alpha = self.sess.run([self.rendered_result, self.rendered_alpha],
                                   feed_dict=fdict)
    ...
    def setup_lf_filter(self):
        ...
        with self.graph.as_default():
            ...
            cvd = tf.squeeze(
                make_psv_homogs(img, pose, newpose, tf.reshape(dispval, [1]),
                                1))
    ...
    def load_graph(self, dirpath, meta_only=False):
        ...
        if self.loaded and self.loaded == ckpt_path:
            ...
            return
        elif self.loaded and self.loaded[:self.loaded.rfind(
                '/')] == ckpt_path[:ckpt_path.rfind('/')]:
            print('Already have graph for {}, loading weights {}', self.loaded,
                  ckpt_path)
        ...
        with self.graph.as_default():
            ...
            outputs = {
                clip_name(clip_name(out.name, '_1'), ':0'): out
                for out in output_
            }
            print('Found outputs:')
    ...
    def run_inference(self,
        ...
        for i, curr_np_inputs in enumerate(test_data):
            ...
            if true_fdicts:
                ...
                cni = [x[np.newaxis, ...]
                       for x in curr_np_inputs[:3]] + curr_np_inputs[3:]
            ...
            if not patched:
                ...
                for i in range(0, sh[0], valid):
                    ...
                    for j in range(0, sh[1], valid):
                        ...
                        w = window[4:8]
                        out_ = {
                            k: out_[k][:, w[0]:w[2], w[1]:w[3], ...]
                            for k in out_
                        }
                        out_row = out_ if out_row is None else {
                            k: np.concatenate([out_row[k], out_[k]], 2)
                            for k in out_row
                        }
                    out = out_row if out is None else {
                        k: np.concatenate([out[k], out_row[k]], 1)
                        for k in out
                    }
